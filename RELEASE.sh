...
stamp_swagger_assets() {
    if ! command -v go >/dev/null 2>&1; then
        echo "go toolchain not found in PATH; cannot stamp swagger version" >&2
        exit 1
    fi

    (
        cd "${PROJECT_ROOT}"
        go run ./cmd/tools/swagger-version-stamp --repo "${PROJECT_ROOT}" --version "${NEW_VERSION}"
    )
}
...
printf 'Swagger metadata stamped for %s\n' "${NEW_VERSION}"
